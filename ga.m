...
for y2=1:VarSize(1)   
    ...
    if(pop(1).Position(y2, y1)>=0.5)
     pop(1).Position(y2,y1)=1;
    else
    pop(1).Position(y2,y1)=0;
end
end
end
    % Truncation
...
spy(pop(1).Position);
aaa=(pop(1).Position);
datae=mean(pdist2(X,aaa)')';
datae_sort=sort(datae);

[center_node1 , center1]=find(datae==datae_sort(1));
[center_node2 , center2]=find(datae==datae_sort(2));
[center_node3 , center3]=find(datae==datae_sort(3));

[center_node1(1) center_node2(1) center_node3(1)]